...
class QuaternionHandler:
    ...
    def quaternion_to_rotationmatrix(self, q):
        w, x, y, z = q
        r00 = 1 - 2*((y**2)+(z**2))
        r01 = 2*(x*y - w*z)
        r02 = 2*(w*y + x*z)
...
def auto_detect_serial(preferred_list=['COM4']):
...
def main():
    ...
    folder_path = "C:/Users/AIuser/Desktop/hip/result/"+ date 
    ...
    # Detect
    available_ports = auto_detect_serial()
    ...
    with open(csv_file, mode='w+', newline='') as file:
        # write title
        write_value[0:7] = 'time','roll(x)(degree)','pitch(y)(degree)','yaw(z)(degree)'
        row = csv.writer(file)
        ...
        try:
            while True:
                if data_thread.data_mpu1 is not None and data_thread.data_mpu2 is not None:
                    ...
                    sum_roll1, sum_pitch1, sum_yaw1 = handler1.get_current_euler()
                    ...
                    sum_roll2, sum_pitch2, sum_yaw2 = handler2.get_current_euler()
                    ...
                    # Update arrow
                    x_arrow1.set_segments([[np.zeros(3), rotation_x * -0.08]])
                    y_arrow1.set_segments([[np.zeros(3), rotation_y * 0.08]])
                    z_arrow1.set_segments([[np.zeros(3), rotation_z * -0.08]])
                    
                    
                    x_arrow2.set_segments([[np.zeros(3),  np.array([1.0, 0.0, 0.0]) * -0.08]])
                    y_arrow2.set_segments([[np.zeros(3),  np.array([0.0, 1.0, 0.0]) * 0.08]])
